...
3. **Jwift Materials** — the visual system. Glass, blur, refraction, shadows, borders — implemented as GPU shaders. The material system from Show Studio's Jiv (`ShowStudio.Web/src/Libraries/Jwift/Jiv/`) is the reference implementation in DOM/CSS. Jwift reimplements these as shader programs.

## Reference Implementation

Show Studio (`show-studio/ShowStudio.Web/src/Libraries/Jwift/`) contains the DOM-based prototype of everything Jwift will do natively:
...
| `GlassDropdown.Component.ts` — dropdown with glass material | `Dropdown` — Jwift primitive with glass material + clip animation |
...
├─────────────────────────────────────────┤
│  Jwift.Angular (or Jwift.React, etc.)   │
...
Jwift's layout engine is a pure-JS flex implementation. No DOM measurement. No `getComputedStyle`. No `offsetWidth`. No forced reflows.
...
The layout from Show Studio's `Jiv.Layout.ts` and `Jiv.Layout.Engine.ts` is the starting point. The difference: Jwift computes layout in pure math, never touching the DOM.
...
Jwift/
  ...
  angular/                 — Jwift.Angular package
    JwiftCanvas.Component.ts
    JwiftPanel.Directive.ts
    JwiftText.Directive.ts
    ...
```
...
### M6 — Angular bindings
`<jwift-canvas>` component. Directives for panels, text, images. Use in Show Studio alongside existing DOM UI.

### M7 — Migration
Replace Jiv in Show Studio with Jwift-rendered equivalents. Page by page. DOM content inside Jwift layout containers.
