...
const _parse = (s: string): Color => {
  ...
  throw new Error(`[Jwift] Unrecognized color: "${s}"`);
};
...
const _parseHex = (s: string): Color => {
  ...
  if (hex.length !== 6 && hex.length !== 8) {
    throw new Error(`[Jwift] Malformed hex color: "${s}" (expected #rgb, #rgba, #rrggbb, or #rrggbbaa)`);
  }
  ...
  if ([r, g, b, a].some(Number.isNaN)) {
    throw new Error(`[Jwift] Invalid hex digits in color: "${s}"`);
  }
...
const _parseHsl = (s: string): Color => {
  ...
  if (open < 0 || close < 0) {
    throw new Error(`[Jwift] Malformed hsl/hsla: "${s}"`);
  }
  const parts = s.slice(open + 1, close).split(/[\s,]+/).filter((p) => p.length > 0);
  if (parts.length !== 3 && parts.length !== 4) {
    throw new Error(`[Jwift] hsl/hsla needs 3 or 4 components: "${s}"`);
  }
...
const _parseRgb = (s: string): Color => {
  ...
  const close = s.indexOf(')');
  if (open < 0 || close < 0) {
    throw new Error(`[Jwift] Malformed rgb/rgba: "${s}"`);
  }
  const inner = s.slice(open + 1, close);
  // Accept comma or whitespace separators (CSS4)
  const parts = inner.split(/[\s,]+/).filter((p) => p.length > 0);
  if (parts.length !== 3 && parts.length !== 4) {
    throw new Error(`[Jwift] rgb/rgba needs 3 or 4 components: "${s}"`);
  }
  ...
  if (raw.some(Number.isNaN)) {
    throw new Error(`[Jwift] Invalid numeric component in color: "${s}"`);
  }
  const [r, g, b, a] = raw;
