...
const _split = (raw: string): _Call[] => {
  ...
  while (i < s.length) {
    ...
    if (name.length === 0) {
      throw new Error(`[Jwift] Unexpected character "${s[i]}" in transform: "${raw}"`);
    }

    // Expect '('
    while (i < s.length && /\s/.test(s[i])) i++;
    if (s[i] !== '(') {
      throw new Error(`[Jwift] Expected "(" after "${name}" in transform: "${raw}"`);
    }
    i++;
...
const _applyCall = (call: _Call, out: Transform, ctx: ResolveContext): void => {
  ...
  switch (n) {
    case 'translate': {
      if (a.length !== 2) throw new Error(`[Jwift] translate() needs 2 args, got ${a.length}`);
      out.TranslateX = Resolve(a[0], ctx, 'W');
      out.TranslateY = Resolve(a[1], ctx, 'H');
      return;
    }
    case 'translatex': {
      if (a.length !== 1) throw new Error(`[Jwift] translateX() needs 1 arg, got ${a.length}`);
      out.TranslateX = Resolve(a[0], ctx, 'W');
      return;
    }
    case 'translatey': {
      if (a.length !== 1) throw new Error(`[Jwift] translateY() needs 1 arg, got ${a.length}`);
    ...
    case 'scale': {
      if (a.length === 1) {
        ...
        throw new Error(`[Jwift] scale() needs 1 or 2 args, got ${a.length}`);
      }
      return;
    }
    ...
    case 'skew': {
      ...
      } else throw new Error(`[Jwift] skew() needs 1 or 2 args, got ${a.length}`);
      return;
    }
    case 'skewx': out.SkewX = Resolve(a[0], ctx, 'W'); return;
    case 'skewy': out.SkewY = Resolve(a[0], ctx, 'H'); return;
    case 'origin': {
      if (a.length !== 2) throw new Error(`[Jwift] origin() needs 2 args, got ${a.length}`);
      out.OriginX = Resolve(a[0], ctx, 'W');
      out.OriginY = Resolve(a[1], ctx, 'H');
      return;
    }
    default:
      throw new Error(`[Jwift] Unknown transform function "${n}"`);
  }
};
