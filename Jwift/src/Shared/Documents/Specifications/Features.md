...
Everything Jwift renders, organized by system.

## Jiv (The Node Primitive)

A Jiv is the Jwift equivalent of a `<div>` — a rectangular node in the scene graph that clips, styles, and contains children. Every visible element is a Jiv or a subtype of one.
...
Icon font rendered as text nodes. The JwiftIcons font uses SF Symbols-like naming (`house.fill`, `chevron.left`).
...
The icon font is a Show Studio asset, not a Jwift-specific dependency. Jwift's text system renders any icon font.

## External Canvas Compositing

Any existing `<canvas>` element (Three.js, WebGL, Canvas2D) can be composited into the Jwift scene as a texture source.

- **Background mode**: external canvas serves as the scene background — Jwift UI layers on top with glass blur sampling the 3D scene beneath
- **Inline mode**: external canvas renders into a node at any position in the tree — behaves like an `<img>` but live-updating
- **Texture source**: the external canvas's pixel data is uploaded as a WebGL texture each frame (or on-demand). Jwift's blur, glass, and refraction shaders sample from it like any other scene content
