...
export class Framebuffer {
  ...
  constructor(gl: WebGL2RenderingContext) {
    ...
    if (!fb) throw new Error('[Jwift] Failed to create framebuffer');
    this.Framebuffer = fb;

    const tex = gl.createTexture();
    if (!tex) throw new Error('[Jwift] Failed to create FBO texture');
    this.Texture = tex;
  }
  ...
  Resize = (width: number, height: number): void => {
    ...
    if (status !== gl.FRAMEBUFFER_COMPLETE) {
      throw new Error(`[Jwift] Framebuffer incomplete: 0x${status.toString(16)}`);
    }
