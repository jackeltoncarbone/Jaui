 ...
 */
const jwiftCapExtent = (y: number): number => {
...
describe('Apple squircle-pill shape fidelity', () => {
  ...
  it('jwift approx matches SS at y=0 (top tip)', () => {
    expect(jwiftCapExtent(0)).toBeCloseTo(ssCapOffset(0), 1);
  });

  it('jwift approx matches SS at y=5', () => {
    expect(Math.abs(jwiftCapExtent(5) - ssCapOffset(5))).toBeLessThan(tolerance);
  });

  it('jwift approx matches SS at y=9 (curve endpoint)', () => {
    expect(Math.abs(jwiftCapExtent(9) - ssCapOffset(9))).toBeLessThan(tolerance);
  });

  it('jwift approx matches SS at y=18 (CP y-value)', () => {
    expect(Math.abs(jwiftCapExtent(18) - ssCapOffset(18))).toBeLessThan(tolerance);
  });

  it('jwift approx matches SS at y=25 (middle max)', () => {
    expect(Math.abs(jwiftCapExtent(25) - ssCapOffset(25))).toBeLessThan(tolerance);
  });

  it('jwift approx matches SS across the full profile within 1.5px', () => {
    let maxErr = 0;
    for (let y = 0; y <= 50; y += 0.5) {
      const err = Math.abs(jwiftCapExtent(y) - ssCapOffset(y));
  ...
  it('jwift approx preserves non-uniform curvature', () => {
    expect(jwiftCapExtent(25)).toBeGreaterThan(jwiftCapExtent(9) * 1.05);
  });
});
