...
Jaui follows C#-style TypeScript conventions, matching the patterns established in Show Studio.
...
In Jaui, this is computed automatically — a child node reads its parent's border radius and subtracts its distance from the parent edge.
...
Components use bare selectors: `panel`, `toolbar`, `dropdown` — not `jaui-panel`, `app-toolbar`.

## Testing

- **Layout math** — unit tests. Pure functions, no DOM, fast.
- **Shader output** — snapshot tests. Render a known scene, compare pixel output.
- **Visual regression** — Playwright screenshots across Chromium + WebKit. The point of Jaui is pixel-identical rendering, so these should match exactly.
