...
Jaui's approach: the layout solver never reads from the DOM. It reads from its own node tree (sizes, constraints, flex properties). It writes target positions. Springs interpolate. The renderer draws. No DOM in the loop at all.
...
- Tracks children via MutationObserver (Jaui won't need this — we own the node tree)
- Measures natural sizes via DOM (Jaui won't need this — sizes come from the style system)
- Runs springs per-child for x, y, w, h, scale, opacity
- Handles entry/exit animation with configurable presence modes

For Jaui, the layout solver (`Jiv.Layout.ts`) ports directly — it's already pure math with no DOM dependency. The engine (`Jiv.Layout.Engine.ts`) gets simplified: no MutationObserver, no ResizeObserver, no DOM measurement. Just node tree → solver → springs → render.
