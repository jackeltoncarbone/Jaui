...
// Hairline floor, in device pixels. MUST match Jiv.Panel.frag's BORDER_MIN_DEVICE_PX — the two
// backends draw the same border and the parity test asserts they carry the same number. The
// derivation is in Jiv.Panel.frag: a stroke thinner than a device pixel can fall between pixel
// centres and paint nothing, so it is drawn at this floor with its lost width carried as coverage.
const BORDER_MIN_DEVICE_PX: f32 = 1.0;
...
const SS_PILL_MAXEXTENT: f32 = 1.5400;

var<private> SS_PILL_CURVE: array<vec2f, 33> = array<vec2f, 33>(
  vec2f(0.00000000, 1.00000000), vec2f(0.04265581, 0.99999445), vec2f(0.08531159, 0.99993035),
  vec2f(0.12796709, 0.99969402), vec2f(0.17062123, 0.99912542), vec2f(0.21327087, 0.99802453),
  vec2f(0.25590906, 0.99615489), vec2f(0.29852244, 0.99324588), vec2f(0.34108789, 0.98899457),
  vec2f(0.38356831, 0.98306800), vec2f(0.42590761, 0.97510672), vec2f(0.46802503, 0.96473086),
  vec2f(0.50980938, 0.95155004), vec2f(0.55111404, 0.93517850), vec2f(0.59175424, 0.91525605),
  vec2f(0.63150841, 0.89147416), vec2f(0.67012530, 0.86360450), vec2f(0.70733754, 0.83152444),
  vec2f(0.74288036, 0.79523315), vec2f(0.77651200, 0.75485312), vec2f(0.80803128, 0.71061553),
  vec2f(0.83728806, 0.66283306), vec2f(0.86418529, 0.61186710), vec2f(0.88867327, 0.55809644),
  vec2f(0.91073888, 0.50189257), vec2f(0.93039268, 0.44360330), vec2f(0.94765574, 0.38354448),
  vec2f(0.96254728, 0.32199785), vec2f(0.97507274, 0.25921346), vec2f(0.98521039, 0.19541521),
  vec2f(0.99289197, 0.13080918), vec2f(0.99796172, 0.06559637), vec2f(1.00000000, 0.00000000)
);
...
fn fs_main(in: VertexOutput) -> @location(0) vec4f {
  ...
  // Hairline floor — never draw a stroke thinner than a device pixel; carry the width it lost as
  // coverage instead. Sits AFTER the variance so variance still thins the rim but can no longer
  // delete it. See Jiv.Panel.frag for the full derivation; the two must agree.
  let varied_border_width = max(local_border_width, 0.0);
  let drawn_border_width = max(varied_border_width, BORDER_MIN_DEVICE_PX);
  let border_coverage = varied_border_width / drawn_border_width;
  ...
  if (material_type == 1.0) {
    ...
    let border_inner = smoothstep(-aa, aa, dist + drawn_border_width);
    // Drawn at the hairline floor, inked by the width it actually has.
    let border_base = (1.0 - border_outer) * border_inner * border_coverage;
