 ...
 * it has. Jaui builds NINETEEN programs before it can draw anything (seven panel variants, text,
 * stroke, two SVG, blit, clip mask, progressive blur, the adaptive-shadow probe, and the blur
 * pass's down/up/copy/mip), so that serialisation is the whole cold-boot shader cost, in a line.
 *
 * NINETEEN IS THE UNFLAGGED SET, and that is the number this class is sized for. A program only a
