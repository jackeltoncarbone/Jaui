...
- Size [C]: the resting pill outset 8 pt a side on a tab bar, 12 pt across and 8 pt down on a segmented control (label-only items), never narrower than the pill, grown by bounds as UIKit grows it, never by transform (`SelectionIndicator.Geometry.ts`). Ours past Apple: it stays concentric with its bar at every size, centred on the bar's height, its corner the bar's less its inset, its ends held at that inset; dragged, the loupe's movement scale narrows it no further than square.
- Springs [C]: the selection's position and bounds on Apple's (Jwift/Apple/Sizing.md 1): dragging 0.85 over 0.2 s and 0.3 s, released 0.85 over 0.4 s and 0.6 s (`SelectionIndicator.ts`, which lays its box out snapped). The lens's own growth and release are fitted to the MacStories 60 fps frames [I]: grow 409 / 25.3 (a 7% overshoot), release 2187 / 112.
