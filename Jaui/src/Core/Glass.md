...
- Refraction: a flat magnifying plate with a bezel-only fold, as Apple's straight, upright labels show. Every pixel of the body reads `centre + (p - centre) / 1.21`, the same in x and y (Apple's Home label reads 1.21 x wider; ours 1.22). Across the bezel, 0.095 of the span (7 pt; the fit to Apple's frames gives 7 pt, ours measures 9), the read runs linearly back to 1 at the outline, so nothing past the lens is ever read into it. The lens stands 1.173 bar heights tall and reads only inside the bar it stands on (1.5 pt inside the bar's edge): past the bar it continues the bar's own body, never the page. The plate is reconstructed by Catmull-Rom, so a magnified glyph stays as sharp as Apple's (ink inside the lens 0.91 as sharp as outside it, on both). Dispersion only in the bezel. The whole refraction fades with the glass, so a release leaves no fold behind.
