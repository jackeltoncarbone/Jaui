...
- Refraction: a flat magnifying plate with a bezel-only fold. Every pixel of the body reads `centre + (p - centre) / 1.21`, the same in x and y (Apple's Home label reads 1.21 x wider; ours 1.22). Across the bezel the read runs on a cubic from 1 / 1.21 out to 1.2 at the outline, pulling what lies past the rim into the band as Apple's mid-drag frames do ("Ho e"; Apple's fit: a 13 pt band reading 1.15 out). Ours keeps the band at 10 pt, 0.135 of the span, because our labels sit nearer the lens's ends than Apple's and a wider band bends them. Each channel folds by its own amount, red furthest: that is the fringe, 0.88 pt at the median in light and 0.11 pt in dark on Apple's frames. The lens stands 1.173 bar heights tall and reads only inside the bar it stands on (1.5 pt inside the bar's edge). The plate is reconstructed by Catmull-Rom, so a magnified glyph stays as sharp as Apple's. The whole refraction fades with the glass, so a release leaves no fold behind.
