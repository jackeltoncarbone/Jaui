...
- Refraction, measured from Apple's own frames (its drag frames' interiors fitted against its resting frame, gradient correlation 0.33): a pixel at depth t inside the outline reads `centre + (p - centre) k(t)`, with k = 1 / 1.21 over the body and `k = mix(1.14, 1 / 1.21, (t / bezel)^3)` across a 13 pt bezel (0.18 of the span). Toward the rim the lens compresses what lies past its outline into the band, reading 1.14 of the way out at the rim, so the bar's own edge shows there. Dispersion only in the bezel. Our live app measures back 1.21, a 15 pt bezel, 1.14 and a cubic curve.
