    ...
    const descendChildren = (node: Jiv, eff: Mat2x3, stack: ClipStack, scope: TeleportScope, effH: Mat3x3 | null, persp: PerspCtx | null): void => {
      ...
      const emitBorderOverlay = (): void => {
        ...
        if (overlayGlass) {
          ...
          // blur/snapshot region below the AABB, clipping the rounded refraction to a rectangle. abs() sizes
          // the region to cover the full displaced footprint so the pill refraction reads valid backdrop.
