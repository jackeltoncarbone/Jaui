...
export class SceneReadLedger {
  ...
  GroupMembers = 0;
  GroupFallbacks = 0;
  /** Vibrancy shape draws this frame (Core/Vibrancy.ts), and vibrant paint draws, panel and text
   *  counted apart. `BlendSwitches` is every blend-state change either made, into its equation and
   *  back, because that is what they cost the batch: each one is a flush on both sides. */
  ShapeDraws = 0;
  BlendDraws = 0;
  BlendSwitches = 0;
