...
describe('the walk, and the flag', () => {
  ...
  it("reuses `?blur-phased`'s traversal rather than inventing a fourth ordering", () => {
    ...
    // FOUR callers now, and the count is the point rather than the number: `?blur-first`'s
    // pre-pass, `_blurPhasedBuild`, `?occlusion`'s pre-pass, and `?glass-group`'s plan scan --
    // each riding the same traversal for the same reason (one answer to "which nodes paint, in
    // what order"). A SECOND ORDERING is what this test refuses, not a further caller of this one.
    expect((JAUI.match(/_blurFirstNode\(this\.Root, MAT_IDENTITY/g) ?? []).length).toBe(4);
  });
